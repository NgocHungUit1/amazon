...
class SaleController extends Controller
{
    ...
    public function indexSaler(Request $request)
    {
        ...
        if ($role->hasPermissionTo('products-index')) {
            ...
            $products = Product::createdByAuthUser()->where('is_active', true)->get();
            ...
            return view('backend.sale.index_saler', compact('warehouse_id', 'all_permission', 'role_id', 'numberOfProduct', 'custom_fields', 'field_name', 'lims_warehouse_list', 'products'));
        } else {
    ...
    public function productSaler($id = '')
    {
        $role = Role::find(Auth::user()->role_id);
        if ($role->hasPermissionTo('sales-add')) {
            $permissions = Role::findByName($role->name)->permissions;
            ...
            if (isset($id)) {
                $lims_sale_data = Sale::find($id);
                $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
                return view('backend.sale.product_saler', compact('lims_sale_data', 'lims_product_sale_data', 'currency_list', 'role', 'all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_reward_point_setting_data', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'options', 'lims_brand_list', 'lims_category_list', 'lims_table_list', 'lims_coupon_list', 'flag', 'numberOfInvoice', 'custom_fields'));
            }

            return view('backend.sale.product_saler', compact('currency_list', 'role', 'all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_reward_point_setting_data', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'options', 'lims_brand_list', 'lims_category_list', 'lims_table_list', 'lims_coupon_list', 'flag', 'numberOfInvoice', 'custom_fields'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function getFeatured(Request $request)
    {
        ...
        $lims_product_list = Product::withoutGlobalScope('createdByAuthUser')
        ...
        foreach ($lims_product_list as $product) {
            if ($product->is_variant) {
                $lims_product_data = Product::withoutGlobalScope('createdByAuthUser')->select('id')->find($product->id);
                $lims_product_variant_data = $lims_product_data->variant()->orderBy('position')->get();
                foreach ($lims_product_variant_data as $key => $variant) {
                    $data['name'][$index] = $product->name . ' [' . $variant->name . ']';
                    $data['code'][$index] = $variant->pivot['item_code'];
