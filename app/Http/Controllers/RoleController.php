    ...
    public function setPermission(Request $request)
    {
        ...
        if ($request->has('products-delete')) {
            ...
            if ($request->has('brand-index')) {
                $permission = Permission::firstOrCreate(['name' => 'brand-index']);
                if (!$role->hasPermissionTo('brand-index')) {
                    $role->givePermissionTo($permission);
                }
            } else
                $role->revokePermissionTo('brand-index');

            if ($request->has('brand-add')) {
                $permission = Permission::firstOrCreate(['name' => 'brand-add']);
                if (!$role->hasPermissionTo('brand-add')) {
                    $role->givePermissionTo($permission);
                }
            } else
                $role->revokePermissionTo('brand-add');

            if ($request->has('brand-edit')) {
                $permission = Permission::firstOrCreate(['name' => 'brand-edit']);
                if (!$role->hasPermissionTo('brand-edit')) {
                    $role->givePermissionTo($permission);
                }
            } else
                $role->revokePermissionTo('brand-edit');

            if ($request->has('brand-delete')) {
                $permission = Permission::firstOrCreate(['name' => 'brand-delete']);
                if (!$role->hasPermissionTo('brand-delete')) {
                    $role->givePermissionTo($permission);
                }
            } else
                $role->revokePermissionTo('brand-delete');

        if ($request->has('products-saler-index')) {
            $permission = Permission::firstOrCreate(['name' => 'products-saler-index']);
            if (!$role->hasPermissionTo('products-saler-index')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('products-saler-index');

        if ($request->has('products-saler-add')) {
            $permission = Permission::firstOrCreate(['name' => 'products-saler-add']);
            if (!$role->hasPermissionTo('products-saler-add')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('products-saler-add');

        if ($request->has('products-saler-edit')) {
            $permission = Permission::firstOrCreate(['name' => 'products-saler-edit']);
            if (!$role->hasPermissionTo('products-saler-edit')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('products-saler-edit');

        if ($request->has('products-saler-delete')) {
            $permission = Permission::firstOrCreate(['name' => 'products-saler-delete']);
            if (!$role->hasPermissionTo('products-saler-delete')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('products-saler-delete');
